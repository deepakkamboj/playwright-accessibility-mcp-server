...
const scanUrlSchema = z.object({
  ...
  waitForPageLoad: z.number().int().positive().default(5000).optional()
    .describe('Time in milliseconds to wait for the page to load before scanning'),
  viewport: z.object({
  ...
  axeOptions: z.object({
    ...
    ]).optional().describe('Standard to test against'),
    rules: z.record(z.string(), z.object({
      enabled: z.boolean()
    })).optional().describe('Enable or disable specific rules'),
  }).optional().describe('Configuration options for axe-core'),
  includeHtml: z.boolean().default(false).optional()
    .describe('Include HTML snippets in the violation reports'),
  maxResults: z.number().int().positive().default(50).optional()
...
const scanHtmlSchema = z.object({
  ...
  waitForPageLoad: z.number().int().positive().default(2000).optional()
    .describe('Time in milliseconds to wait for the page to load before scanning'),
  viewport: z.object({
    width: z.number().int().positive().default(1280).optional(),
    height: z.number().int().positive().default(720).optional(),
...
const scanBatchSchema = z.object({
  urls: z.array(z.string().url({ message: "Must be a valid URL starting with http:// or https://" }))
...
const summariseViolationsSchema = z.object({
  violations: z.array(z.object({
    id: z.string(),
    impact: z.union([z.literal("minor"), z.literal("moderate"), z.literal("serious"), z.literal("critical")]),
  ...
  format: z.union([z.literal("default"), z.literal("simple"), z.literal("detailed"), z.literal("markdown"), z.literal("html"), z.literal("csv")])
    .default("default").optional().describe("Output format for the summary"),
});
...
async function performA11yScan(
  ...
  const tags = params.axeOptions?.runOnly ? [params.axeOptions.runOnly] : ['wcag2a', 'wcag2aa', 'wcag21aa']; // Get tags from axeOptions
  const builder = new axeBuilder({ page }).withTags(tags);

  
  const accessibilityScanResults = await builder.analyze();
  const violations = accessibilityScanResults.violations.slice(0, params.maxResults || 50).map((violation: any) => ({
    id: violation.id,
...
export function registerScanUrlTool(server: McpServer) {
  server.tool(
    ...
    async (params: z.infer<typeof scanUrlSchema>) => {
      try {
        logMessage("info", `Starting accessibility scan for URL: ${params.url}`);
        ...
        try {
          ...
          const results = await performA11yScan(page, params, `scan-url-${params.url}`);
  ...
  server.tool(
    ...
    async (params: z.infer<typeof scanHtmlSchema>) => {
      try {
        ...
        try {
          const page = await context.newPage();
          await page.setContent(params.html, { waitUntil: "networkidle", timeout: 30000 });

          if (params.waitForPageLoad) {
            await page.waitForTimeout(params.waitForPageLoad);
          }

          const results = await performA11yScan(page, { ...params, url: params.html }, "scan-html");
...
export function registerScanBatchTool(server: McpServer) {
  server.tool(
    ...
    async (params: z.infer<typeof scanBatchSchema>) => {
      try {
        logMessage("info", `Starting batch accessibility scan for ${params.urls.length} URLs`);
        ...
        try {
          for (const url of params.urls) {
            ...
            try {
              ...
              const result = await performA11yScan(page, { ...params, url }, `scan-batch-${url}`);
              results.push({ url, success: true, ...result });
              logMessage("info", `Scan completed for URL: ${url}`);
            } catch (error) {
              logMessage("error", `Error scanning URL ${url}: ${(error as Error).message}`);
        ...
        logMessage("info", `Batch scan completed for ${params.urls.length} URLs`);
        ...
        logMessage("error", `Batch scanning error: ${(error as Error).message}`);
        return {
          content: [
            {
              type: "text",
...
export function registerSummariseViolationsTool(server: McpServer) {
  server.tool(
    ...
    async (params: z.infer<typeof summariseViolationsSchema>) => {
      try {
        logMessage("info", `Summarizing ${params.violations.length} accessibility violations`);
        ...
        logMessage("info", `Summary generated for ${params.violations.length} violations`);
        ...
        logMessage("error", `Error summarizing violations: ${(error as Error).message}`);
        return {
          content: [
            {
              type: "text",
              text: `Error summarizing violations content: ${(error as Error).message}`,
            },
          ],
          isError: true,
        };
...
export function registerWriteViolationsTool(server: McpServer) {
  server.tool(
    ...
    async (params: z.infer<typeof summariseViolationsSchema>) => {
      try {
        logMessage("info", `Writing ${params.violations.length} accessibility violations report`);
        const summary = params.violations.reduce((acc, violation) => {
          acc[violation.impact] = (acc[violation.impact] || 0) + 1;
          return acc;
        }, {} as Record<string, number>);
